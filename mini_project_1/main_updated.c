...
2. Autocomplete (using NCurses)
3. Suggestion Search (using Soundex)
...
#define runtime ((double)(clock()-start))/CLOCKS_PER_SEC;
#define computing\
		cpuTimeUsed = runtime;\
		if(cpuTimeUsed < 0.000001)\
			printf("\n>>Function's runtime is too short, not considerable (< 10^-6 seconds)!.\n");\
		else\
			printf("\n>>Function's runtime: %6lf seconds.\n", cpuTimeUsed);
...
int main(){
    ...
    btcls(dictionary);
    if((double)(clock() - sysStart)/CLOCKS_PER_SEC < 0.000001)
		printf("\n>>Program's total runtime is too short, not considerable (< 10^-6 seconds)!.\n");
	else
		printf("\n>>Program's total runtime: %6lf\n", (double)(clock() - sysStart)/CLOCKS_PER_SEC);
