...
BUGS:
- PriorityQueue bị core dumped

FIXED:
- Replace jval_iarray with jval_v to fix PriorityQueue
*/
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
...
HGraph createHGraph();
void addVertex(HGraph graph, int id, int letter);
int getVertex(HGraph graph, int id);
void addEdge(HGraph graph, int v1, int v2, int weight);
int getEdgeValue(HGraph graph, int v1, int v2);
int indegree(HGraph graph, int v, int* output);
int outdegree(HGraph graph, int v, int* output);
void dropHGraph(HGraph graph);
...
void addVertex(HGraph g, int id, int letter){
     JRB node = jrb_find_int(g.vertices, id);
     if (node==NULL) // only add new vertex 
         jrb_insert_int(g.vertices, id, new_jval_i(letter));            
}

int getVertex(HGraph g, int id){
     JRB node = jrb_find_int(g.vertices, id);
     if (node==NULL) 
        return 0;
     else                
...
// 		node = dll_next(node);
// 	}
// 	if(node==q) dll_append(q, new_jval_iarray(graphnode, frequence));
// 	else dll_insert_b(node, new_jval_iarray(graphnode, frequence));	
// }

void add2Queue(PriorityQueue q, int graphnode, int frequence){
	Dllist node = dll_first(q);
	while(node != q){
		if(frequence <= ((int*)jval_v(node->val))[1]) break;
		node = dll_next(node);
	}
	...
	item[1] = frequence;
	if(node == q){
		dll_append(q, new_jval_v(item));
	}
	else{
		dll_insert_b(node, new_jval_v(item));
	}
}
...
HuffmanTree makeHuffman(int fTable[]) {
	...
	for (;;){
		...
		lastNodeID++;
		addVertex(hTree.graph, lastNodeID, -1);
		addEdge(hTree.graph, lastNodeID, ((int*)jval_v(dll_val(n1)))[0], 0); 
		addEdge(hTree.graph, lastNodeID, ((int*)jval_v(dll_val(n2)))[0], 1);
		// modify the queue
		dll_delete_node(n1);
		dll_delete_node(n2);
		add2Queue(queue, lastNodeID, ((int*)jval_v(dll_val(n1)))[1]+((int*)jval_v(dll_val(n2)))[1]);
	}
	n1 = dll_first(queue);
	if ( n1==queue ) 
		hTree.rootID = 0;
	else
		hTree.rootID = ((int*)jval_v(dll_val(n1)))[0];	
...
void getCode(int node, HGraph g, int len, char code[], Coding htable[]) {
	int ch, k, i;
	int output[10];
	ch = getVertex(g, node);
	if (ch != -1) {
